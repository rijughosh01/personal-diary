...
function DiaryList({ entries, setEntries }) {
  ...
  const [editedEntry, setEditedEntry] = useState({ entry: '', tags: [], mood: '' });
  ...
  return (
    <div>
      <Typography variant="h5" mb={2}>Diary Entries</Typography>
      {entries.map((entry, index) => (
        <motion.div key={index} className="entry" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
          {editIndex === index ? (
            <Box sx={{ p: 2, borderRadius: 2, boxShadow: 3, backgroundColor: '#f9f9f9' }}>
              <ReactQuill
            ...
            <Box sx={{ p: 2, borderRadius: 2, boxShadow: 1, backgroundColor: '#f9f9f9' }}>
              <Typography variant="body1" dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(entry.entry) }}></Typography>
              <Typography variant="body2">Tags: {entry.tags.join(', ')}</Typography>
